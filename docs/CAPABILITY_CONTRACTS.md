...
每个 Contract 用同一组六项描述一个位置的正向能力。没有写入 `Owns` 的决定不属于该位置；没有写入 `Capabilities` 的动作不应由该位置自行取得。遇到自身能力之外的问题，按 `Handoff` 交回有权位置。
...
- `projectAccess=none` 时不取得项目输入；`read` 只允许读取所选项目；`write` 只有在 execution Task 中才可请求所选项目的写能力。

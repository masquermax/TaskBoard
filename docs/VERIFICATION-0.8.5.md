# v0.8.5 Release Verification

Status: VERIFIED IN LOCAL TEST ENVIRONMENT — WINDOWS OA PENDING

v0.8.5 is a runtime subtraction release driven by the real Windows v0.8.4 logs. It does not add a new scheduler or user concurrency setting. It separates model identity from model-catalog enhancement work, separates retry waiting from resource waiting, and makes real Codex Turn concurrency observable.
...
- Refresh is single-flight and atomic. Failure preserves the model/catalog record present before refresh. If no model record exists, the state remains explicit Executor Default fallback.
...
- `WAITING_RESOURCE` means execution capacity was not obtained. `RETRY_WAIT` means an execution failed and is waiting for its next attempt.

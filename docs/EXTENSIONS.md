...
An Extension factory may return:
...
- `surfaceHosts[]` are optional interaction surfaces and may coexist.
...
- `surfaceHosts[]` may have multiple simultaneous contributors.
...
1. the external registry can bootstrap the Extension without Core changes;
2. required TaskBoard Work/Authority/Context semantics can be realized without weakening them;
3. model/capability and connection presentation remain normalized at the boundary;
4. a real Runtime turn completes on the intended value path;
5. removing the Extension leaves stock TaskBoard semantics and tests valid.

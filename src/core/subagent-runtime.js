...
function uniqueStrings(value){return[...new Set(list(value).map(text).filter(Boolean))];}
...
function bindEvidenceToWorkSubject(evidence,delegation){
  const subjectRefs=uniqueStrings(delegation?.subjectRefs);
  return list(evidence).map(item=>{
    const copy={...(item||{})};
    // Work-subject provenance is Runtime-owned transport metadata. Keep it
    // non-enumerable so it can be checked by Validator during this Root turn
    // without becoming a second persisted Evidence field or model-authored fact.
    Object.defineProperty(copy,'_workSubjectRefs',{value:subjectRefs,enumerable:false,writable:false,configurable:false});
    return copy;
  });
}
...
export class SubagentRuntime{
  ...
  async run(task,delegation,{onProgress=null,onExecutionStarted=null,signal=null,policyContext=null}={}){
    ...
    const evidence=bindEvidenceToWorkSubject(raw?.evidence,delegation),blocker=text(raw?.blocker)||null;

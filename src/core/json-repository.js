...
export class JsonTaskRepository {
  ...
  setAnalysisState(id,state){const t=this.state.tasks.find(x=>x.id===id);if(!t)throw new Error('TASK_NOT_FOUND');this.store.transaction(()=>{t.analysis_state=state==null?null:clone(state);});return this.getTask(id);}
  ...
  addProgressHistory(taskId,{title,detail='',completedAt=null}){this.store.transaction(()=>this.state.progressHistory.push({id:++this.state.counters.progress,task_id:taskId,title,detail,completed_at:completedAt||this.now()}));}

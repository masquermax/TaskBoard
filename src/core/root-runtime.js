...
function rootActivityCopy(kind='initial'){
  const copy={
    ...
    synthesis:{title:'Root 综合结果',waiting:'当前 Work Unit 批次已结束；正在获取 Root 综合资源。',running:'Root 正在把本批新结果与当前认知推到本轮闭包，并形成下一动作。'},
    triggered:{title:'Root 继续判断',waiting:'新的 Task 触发已就绪；正在获取 Root 资源。',running:'Root 正在基于当前真实状态继续推进。'},
  };
...
function validationError(violations=[]){const summary=list(violations).slice(0,6).map(v=>`${v.ruleId}:${v.target}:${v.reason}`).join(' | ');const error=new Error(`GOVERNANCE_VALIDATION_FAILED${summary?`: ${summary}`:''}`);error.nonRetryable=true;error.governanceViolations=violations;return error;}
...
function rootCertifiedProjection(task,session){
  const current=session?.certifiedContext||{};
  const claims=clone(list(current.claims));
  const gaps=clone(list(current.gaps));
  ...
  const obligations=list(task?.taskContract?.obligations)
  ...
  return{version:Number(session?.analysisState?.version)||0,claims,gaps,unresolvedObligations:obligations};
}
...
export function validateDelegationPlan(delegations,{knownWorkIds=[],availableInputRefs=null}={}){
  ...
  const knownIds=new Set(list(knownWorkIds).map(text).filter(Boolean)),allowedInputs=Array.isArray(availableInputRefs)?new Set(availableInputRefs.map(text).filter(Boolean)):null,ids=new Set();
  ...
  for(const item of selected){
    if(!item.id)continue;if(item.dependsOn.includes(item.id))issues.push(`工作 ${item.id} 不能依赖自身。`);
...
export class RootRuntime{
  ...
  async certifyWorkAuthority(task,callbacks,workUnits=[]){
    ...
    const reviews=list(result?.reviews),nextContract=applyAuthorityFidelity(task.taskContract,candidates,reviews);callbacks.onTaskContractAuthority?.(nextContract.authority);return{...task,taskContract:nextContract};
  }

  createSession(task){
    const restoredAnalysisState=normalizeCertifiedState(task.analysisState),durableWorkReceipts=list(task.workReceipts).filter(receipt=>receipt?.signature&&receipt?.workUnit&&receipt?.result),pendingWorkResults=durableWorkReceipts.filter(receipt=>!receipt.consumed_at).map(receipt=>({...clone(receipt.result),workUnit:clone(receipt.workUnit),persistedReceipt:true}));
    const session={
  ...
  async runRootTurn(task,session,callbacks,{humanGatewayHistory=[],rootInputs=null,activityKind='initial'}={}){
    ...
    const activity=rootActivityCopy(activityKind),issuedAt=nowIso();session.actor={title:activity.title,status:WorkUnitStatus.WAITING_RESOURCE,detail:activity.waiting,issuedAt,startedAt:null,completedAt:null,updatedAt:issuedAt,owner:'root'};this.emit(session,callbacks);
    const controller=new AbortController(),deliveredResults=Array.isArray(rootInputs)?rootInputs:session.subagentResults.slice(),activeWork=session.currentStage?session.currentStage.workUnits.map(unit=>({id:unit.id,title:unit.title,status:unit.status,projectAccess:unit.projectAccess||'none',networkAccess:unit.networkAccess===true,dependsOn:unit.dependsOn})):[];
    const rootTask=rootTaskProjection(task),certifiedContext=rootCertifiedProjection(task,session);session.rootController=controller;
    try{
      await this.modelRouter.prepare?.({role:'root',task:rootTask});
      const decision=normalizeDecision(await this.executor.runRoot({task:rootTask,subagentResults:deliveredResults,activeWork,humanGatewayHistory,modelPolicy:this.modelRouter.route({role:'root',task:rootTask}),policyContext:this.governanceCompiler?.compileForRole?.(task,'root')||session.policyContext,certifiedContext,signal:controller.signal,onExecutionStarted:()=>{const startedAt=nowIso();session.actor.status=WorkUnitStatus.RUNNING;session.actor.startedAt=session.actor.startedAt||startedAt;session.actor.detail=activity.running;session.actor.updatedAt=startedAt;callbacks.onExecutionStarted?.({role:'root'});this.emit(session,callbacks);},onProgress:progress=>{session.actor.detail=progress.detail||progress.summary||session.actor.detail;session.actor.updatedAt=nowIso();this.emit(session,callbacks);}}));
  ...
  async reviewRootDecision(task,session,decision,callbacks,{humanGatewayHistory=[],rootInputs=[],triggerRefs=[]}={}){
    ...
    const violations=list(prepared.issues).map(issue=>({ruleId:'C-003',target:issue.target||'state',reason:issue.reason,action:issue.code})),blockingGap=prepared.current.gaps?.find?.(gap=>gap?.blocking===true),gatewayGapId=text(reviewed.decision?.gateway?.gapId),gatewayGap=list(prepared.current.gaps).find(gap=>text(gap?.id)===gatewayGapId)||null,normalizeQuestion=value=>text(value).replace(/\s+/g,' ');
  ...
  buildWorkUnits(stage,delegations){
    const existingIds=new Set(list(stage?.workUnits).map(unit=>unit.id));
    return list(delegations).map((d,index)=>{
      const id=String(d.id),deps=[...new Set(list(d.dependsOn).map(String))].filter(x=>x!==id),waitingOnDependency=deps.some(dep=>{const prior=stage?.workUnits?.find(unit=>unit.id===dep);return !prior||prior.status!==WorkUnitStatus.COMPLETED;}),issuedAt=nowIso();
      return{id,title:String(d.title||`工作 ${index+1}`),goal:String(d.goal||''),expectedOutput:String(d.expectedOutput||''),stopCondition:String(d.stopCondition||''),projectAccess:['read','write'].includes(d.projectAccess)?d.projectAccess:'none',networkAccess:d.networkAccess===true,inputRefs:[...list(d.inputRefs)],skillId:d.skillId||null,dependsOn:deps,status:waitingOnDependency?WorkUnitStatus.WAITING_DEPENDENCY:WorkUnitStatus.WAITING_RESOURCE,detail:waitingOnDependency?'等待前置工作完成后继续。':'工作已就绪，等待可用 Agent。',issuedAt,startedAt:null,updatedAt:issuedAt,completedAt:null,failureCount:0,nextRetryAt:Date.now(),result:null,owner:null,effectRecoveryRequired:false};
  ...
  startSubagent(task,session,unit,callbacks){
    ...
    const dependencyResults=unit.dependsOn.map(id=>{const dep=session.currentStage?.workUnits.find(x=>x.id===id);return dep?.result?{id,title:dep.title,result:dep.result}:null;}).filter(Boolean),workUnit={id:unit.id,title:unit.title,goal:unit.goal,expectedOutput:unit.expectedOutput,stopCondition:unit.stopCondition,projectAccess:unit.projectAccess||'none',networkAccess:unit.networkAccess===true,skillId:unit.skillId,dependsOn:[...unit.dependsOn],inputRefs:[...unit.inputRefs]};
    ...
    const promise=this.subagentRuntime.run(task,{...workUnit,dependencyResults},{signal:controller.signal,policyContext:this.governanceCompiler?.compileForRole?.(task,'subagent',{skillId:unit.skillId,workUnit:unit})||session.policyContext,onExecutionStarted:()=>{executionStarted=true;const startedAt=nowIso();unit.status=WorkUnitStatus.RUNNING;unit.owner='subagent';unit.detail=unit.failureCount?`正在进行第 ${unit.failureCount+1}/${MAX_TOTAL_ATTEMPTS} 次尝试。`:'正在执行分配的具体工作。';unit.startedAt=unit.startedAt||startedAt;unit.updatedAt=startedAt;callbacks.onExecutionStarted?.({role:'subagent',workUnitId:unit.id});this.emit(session,callbacks);},onProgress:progress=>{unit.owner='subagent';unit.detail=progress.detail||progress.summary||unit.detail;unit.updatedAt=nowIso();this.emit(session,callbacks);}})
      ...
      .catch(error=>{
        ...
        if(effectCapable&&!executionStarted&&!clearSafeAdmission())return;unit.failureCount+=1;unit.owner='subagent';
  ...
  async runStage(task,session,callbacks){
    ...
    while(true){
      ...
      if(runningPromises.length){const nextRetryAt=stage.workUnits.filter(x=>[WorkUnitStatus.WAITING_RESOURCE,WorkUnitStatus.RETRY_WAIT].includes(x.status)&&!session.runningPromises.has(x.id)&&x.nextRetryAt).map(x=>Number(x.nextRetryAt)).filter(Number.isFinite).sort((a,b)=>a-b)[0],waits=runningPromises.map(p=>p.catch(()=>null));if(nextRetryAt){const delay=Math.max(0,nextRetryAt-Date.now());waits.push(new Promise(resolveWait=>{const timer=setTimeout(resolveWait,delay);timer.unref?.();}));}await Promise.race(waits);continue;}
      if(stage.workUnits.every(x=>x.status===WorkUnitStatus.COMPLETED)){const completedUnits=stage.workUnits.map(unit=>({title:unit.title,detail:unit.detail,completedAt:unit.completedAt||unit.updatedAt}));callbacks.onStageCompleted?.(completedUnits);session.completedWorkUnits.push(...stage.workUnits.map(unit=>snapshotWorkUnit(unit,stage.id)));session.currentStage=null;session.round+=1;this.emit(session,callbacks);return{kind:'stage_complete'};}
      ...
      const future=stage.workUnits.filter(x=>[WorkUnitStatus.WAITING_RESOURCE,WorkUnitStatus.RETRY_WAIT].includes(x.status)&&x.nextRetryAt).map(x=>x.nextRetryAt);if(future.length){const retrying=stage.workUnits.some(x=>x.status===WorkUnitStatus.RETRY_WAIT&&x.nextRetryAt);return{kind:retrying?'retry_wait':'waiting_resource',retryAt:Math.min(...future),snapshot:this.makeSnapshot(session),reason:retrying?'等待自动重试':'等待执行资源恢复'};}
  ...
  async execute(task,{humanGatewayHistory=[],onProgress=null,onStageCompleted=null,onCertifiedTurn=null,onTaskContractAuthority=null,onWorkReceipt=null,onWorkReceiptsConsumed=null,onEffectAttempt=null,onEffectAttemptCleared=null,onExecutionStarted=null}={}){
    ...
    while(true){
      ...
      try{decision=await this.runRootTurn(task,session,callbacks,{humanGatewayHistory:humanHistoryForTriggerRefs(humanGatewayHistory,rootTriggerRefs),rootInputs,activityKind});}catch(error){if(isCapacityUnavailable(error)&&rootInputs.length){const outcome=await capacityWait({title:'Root 综合结果',detail:'Work Unit 批次结果已保留；等待 Root 资源。',reason:'等待 Root 综合资源恢复'});if(outcome)return outcome;continue;}throw error;}

...
export class RootRuntime{
  ...
  startSubagent(task,session,unit,callbacks){
    ...
    const dependencyResults=unit.dependsOn.map(id=>{const dep=session.currentStage?.workUnits.find(work=>work.id===id);return dep?.result?{id,title:dep.title,result:dep.result}:null;}).filter(Boolean),workUnit={id:unit.id,title:unit.title,goal:unit.goal,expectedOutput:unit.expectedOutput,stopCondition:unit.stopCondition,obligationRefs:[...list(unit.obligationRefs)],projectAccess:unit.projectAccess||'none',networkAccess:unit.networkAccess===true,skillId:unit.skillId,dependsOn:[...unit.dependsOn],inputRefs:[...unit.inputRefs]};

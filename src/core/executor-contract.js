...
import { certifiedCognitionReuseInstructions, projectCertifiedKnownClaims, rootRealityContinuityInstructions } from './certified-cognition-reuse.js';
...
export const ROOT_RESPONSE_CONTRACT={
  ...
  properties:{
    ...
    delegations:{type:'array',items:{type:'object',properties:{id:{type:'string'},title:{type:'string'},goal:{type:'string'},expectedOutput:{type:'string'},stopCondition:{type:'string'},obligationRefs:{type:'array',items:{type:'string'},maxItems:20},subjectRefs:{type:'array',items:{type:'string'},maxItems:12},projectAccess:{type:'string',enum:['none','read','write']},networkAccess:{type:'boolean'},skillId:{type:['string','null']},dependsOn:{type:'array',items:{type:'string'}},inputRefs:{type:'array',items:{type:'string'},maxItems:40}},required:['id','title','goal','expectedOutput','stopCondition','obligationRefs','subjectRefs','projectAccess','networkAccess','skillId','dependsOn','inputRefs'],additionalProperties:false}},
...
function governedParentContext(task){
  const contract=task?.taskContract??task?.task_contract??null;
  if(!contract||typeof contract!=='object')return{contractId:null,revision:null,obligations:[],constraints:[]};
  return{
    contractId:text(contract?.id)||null,
    revision:Number.isFinite(Number(contract?.revision))?Number(contract.revision):null,
    obligations:clone(list(contract?.obligations)),
    constraints:clone(list(contract?.constraints)),
  };
}

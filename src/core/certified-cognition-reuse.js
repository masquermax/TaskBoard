...
export function rootRealityContinuityInstructions(){
  return [
    ...
    'For a Claim with explicit factProperty/factValue, same subject + scope + factProperty is that factual slot. Fresh DIRECT Evidence may revise its factValue even when wording or proposed Claim id differs; Runtime should preserve one active current value and keep the prior value only in turn history.',
  ].join(' ');
}

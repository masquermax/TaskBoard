...
export class CodexAppServerClient{
  ...
  async runTurn({cwd,writableRoots=[],prompt,inputItems=[],outputSchema,model=null,reasoningEffort=null,networkAccess=false,permissionProfile=null,runtimeWorkspaceRoots=[],environments=null,runtimeConfig=null,onProgress=null,onExecutionStarted=null,signal=null,diagnosticContext=null}){
    const isWorkUnit=Boolean(diagnosticContext?.workUnitId),roleLabel=isWorkUnit?'Work Unit':'Task 判断',runningDetail=isWorkUnit?'模型正在执行当前 Work Unit。':'模型正在进行 Task 级判断。',formedDetail=isWorkUnit?'正在等待当前 Work Unit 完成并交回 Root。':'正在等待本轮 Root 判断完成。',completedDetail=isWorkUnit?'Work Unit 结果已交回 Root。':'Root 本轮判断已完成。',commandDetail=isWorkUnit?'正在检查当前 Work Unit 授权输入中的证据。':'正在处理 TaskBoard 临时工作区中的本轮判断材料。',fileChangeDetail=isWorkUnit?'正在修改当前 Work Unit 明确授权的文件范围。':'正在处理 TaskBoard 临时工作区文件。';
    if(signal?.aborted){const error=new Error('Execution interrupted');error.interrupted=true;throw error;}
    const executionStartedAt=Date.now(),routeMeta={taskId:diagnosticContext?.taskId||null,workUnitId:diagnosticContext?.workUnitId||null,routeReason:diagnosticContext?.routeReason||null,requestedModel:model||null,configuredDefaultModel:diagnosticContext?.configuredDefaultModel||null,reasoningEffort:reasoningEffort||null,inputBytes:Buffer.byteLength(String(prompt||''),'utf8')};
    ...
    const workUnitObserver=isWorkUnit?registerWorkUnitObservability(new WorkUnitObservability({taskId:routeMeta.taskId,workUnitId:routeMeta.workUnitId,turnId,startedAt:Date.now(),emitDiagnostic:(event,data,level)=>this.recordDiagnostic(event,data,level)})):null;

...
function errorStatus(message){
  ...
  if(['TASK_CANCEL_NOT_ALLOWED','TASK_DELETE_BECAME_RUNNING','TASK_DELETE_NOT_ALLOWED','TASK_LOCKED','TASK_LOCK_NOT_ALLOWED','TASK_NOT_QUIESCENT','TASK_RETRY_NOT_ALLOWED','RETRY_TARGET_NOT_SUSPENDED','MODEL_SELECTION_SCOPE_UNAVAILABLE','MODEL_SELECTION_CATALOG_UNAVAILABLE','MODEL_SELECTION_MODEL_UNAVAILABLE','MODEL_SELECTION_EXPLICIT_UNSUPPORTED'].includes(message))return 409;
...
export function createApp({taskService,executor,scheduler=null,capabilityProvider=null,surfaceManager=null,extension=null,settingsStore=null,runtimeSettingsState=null,applyRuntimeSettings=null,modelSelectionState=null,applyModelSelection=null,uiRoot,onShutdown=null,instanceRoot=null}){
  return async function handler(req,res){
    try{
      ...
      if(path==='/api/model-selection'&&req.method==='GET'){if(!modelSelectionState)return json(res,503,{error:'MODEL_SELECTION_UNAVAILABLE'});return json(res,200,{modelSelection:modelSelectionState()});}
      if(path==='/api/model-selection'&&req.method==='PUT'){if(!applyModelSelection)return json(res,503,{error:'MODEL_SELECTION_UNAVAILABLE'});return json(res,200,{modelSelection:applyModelSelection(await readJson(req))});}

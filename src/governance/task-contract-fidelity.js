...
function stripNegatedMutation(value){return text(value).replace(/(?:不|不要|不得|无需|不需要|禁止|只读|仅查看|仅检查)\s*(?:进行)?\s*(?:修改|修复|开发|实现|部署|安装|删除|重构|提交|打包|发布|升级|改造|写代码|写入)[^，。；;\n]*/gi,' ');}
function explicitProjectWrite(value){const source=stripNegatedMutation(value);return /(?:请|帮我|直接|现在|开始|需要|要求|把|给我|完成)?\s*(?:开发|修复|修改|新增|删除|重构|部署|安装|提交|发布|升级|改造|写代码|写入|实现(?:功能|需求|逻辑|代码|方案|改造))/i.test(source);}
function stripNegatedNetwork(value){return text(value).replace(/(?:不|不要|不得|无需|不需要|禁止)\s*(?:联网|访问(?:网络|互联网)|上网|使用网络|网络访问|搜索网页|搜索网络)[^，。；;\n]*/gi,' ');}
function explicitNetworkAccess(value){const source=stripNegatedNetwork(value);return /(?:请|帮我|直接|现在|开始|需要|允许|要求)?\s*(?:联网|访问(?:网络|互联网)|上网|使用网络|网络访问|搜索网页|搜索网络)|\b(?:search the web|use the network|internet access|online lookup)\b/i.test(source);}

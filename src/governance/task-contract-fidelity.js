...
function mutationDenied(value){const source=text(value);return /(?:不|不要|不得|无需|不需要|不用|不必|禁止|不可)\s*(?:实际)?\s*(?:进行)?\s*(?:修改|修复|开发|实现|部署|安装|删除|重构|提交|打包|发布|升级|改造|写代码|写入|变更|执行)/i.test(source)||/(?:只读|仅查看|仅检查|只分析|仅分析|只解释|仅解释|只说明|仅说明)/i.test(source);}
function networkDenied(value){const source=text(value);return /(?:不|不要|不得|无需|不需要|不用|不必|禁止|不可)\s*(?:进行)?\s*(?:联网|访问(?:网络|互联网)|上网|使用网络|网络访问|搜索网页|搜索网络)/i.test(source)||/(?:只|仅)\s*(?:看|查|检查|分析|读取)?\s*(?:本地|项目|附件)/i.test(source);}

...
test('new task enters READY with NEW marker; Human Gateway reply returns READY with HUMAN_REPLY marker',async()=>{
  const x=setupScheduler();try{const task=x.scheduler.createTask({title:'做一个 OA 系统',instruction:'你帮我做了吧'});assert.equal(task.status,TaskStatus.READY);assert.equal(task.ready_reason,ReadyReason.NEW);await x.scheduler.tick();assert.equal(x.service.getTask(task.id).status,TaskStatus.WAITING_HUMAN);const replied=x.scheduler.answerHumanGateway(task.id,'基础办公');assert.equal(replied.status,TaskStatus.READY);assert.equal(replied.ready_reason,ReadyReason.HUMAN_REPLY);}finally{x.close();}
});

test('status lists sort latest activity descending while scheduler claims oldest READY first',()=>{
  let now=new Date('2026-08-07T12:00:00.000Z');const dir=mkdtempSync(join(tmpdir(),'taskboard-order-'));const db=new JsonTaskDatabase(join(dir,'db.json'));const repo=new JsonTaskRepository(db);repo.now=()=>now.toISOString();const service=new TaskService(repo);try{const a=service.createTask({title:'A',instruction:'A'});now=new Date('2026-08-07T12:01:00.000Z');const b=service.createTask({title:'B',instruction:'B'});assert.deepEqual(service.listTasks({status:TaskStatus.READY}).map(t=>t.id),[b.id,a.id]);assert.deepEqual(repo.listRunnableTasks(10).map(t=>t.id),[a.id,b.id]);}finally{db.close();rmSync(dir,{recursive:true,force:true});}
});

test('RUNNING means a Task execution slot is actually occupied and never exceeds configured concurrency',async()=>{
  let releases=[],started=0;const executor={async runRoot({onExecutionStarted}){started++;onExecutionStarted?.();return new Promise(resolve=>releases.push(()=>resolve(complete())));},async runSubagent(){throw new Error('unused');}};const x=setupScheduler(executor,{maxConcurrentTasks:2});try{for(let i=0;i<5;i++)x.scheduler.createTask({title:`T${i}`,instruction:'work'});const tick=x.scheduler.tick();for(let i=0;i<50&&started<2;i++)await new Promise(r=>setTimeout(r,5));assert.equal(started,2);assert.equal(x.service.counts().RUNNING,2);assert.equal(x.service.counts().READY,3);releases.splice(0).forEach(fn=>fn());await tick;assert.equal(x.service.counts().COMPLETED,2);}finally{x.close();}
});

test('non-retryable executor protocol error suspends after the first failed attempt and never loops',async()=>{
  let calls=0;const executor={async runRoot(){calls++;const e=new Error('Invalid request: unknown variant workspaceWrite');e.nonRetryable=true;throw e;},async runSubagent(){throw new Error('unused');}};const x=setupScheduler(executor);try{const task=x.scheduler.createTask({title:'协议错误',instruction:'不要反复消耗'});await x.scheduler.tick();const current=x.service.getTask(task.id);assert.equal(calls,1);assert.equal(current.status,TaskStatus.READY);assert.equal(current.ready_reason,ReadyReason.SUSPENDED);assert.equal(current.executionState.retry.failureCount,1);await x.scheduler.tick();assert.equal(calls,1);}finally{x.close();}
});

test('startup recovery is performed by Scheduler and reconciles stale RUNNING back to READY',()=>{
  const x=setupScheduler();try{const task=x.scheduler.createTask({title:'中断任务',instruction:'恢复'});x.repo.transitionTask(task.id,TaskStatus.RUNNING);assert.equal(x.scheduler.recoverStaleRunningTasks(),1);assert.equal(x.service.getTask(task.id).status,TaskStatus.READY);}finally{x.close();}
});

...
test('full task flow: project -> attachment task -> completion -> search -> reference',async()=>{
  ...
  try{
    ...
    const form=new FormData();form.append('title','做一个 OA 系统');form.append('instruction','验证通用 Executor 控制链');form.append('projectId',project.id);form.append('attachments',new Blob(['OA background material'],{type:'text/plain'}),'背景资料.txt');
    ...
    await runtime.scheduler.tick();const completed=runtime.taskService.getTask(created.id);assert.equal(completed.status,'COMPLETED');

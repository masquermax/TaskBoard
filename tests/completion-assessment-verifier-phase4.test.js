...
test('Completion assessment consumes Root-owned obligation mapping and CompletionEvaluator remains the only aggregator',async()=>{
  ...
  const result=await verifier.review({task:task(),proposal:{finalResult:'只完成 A',summary:'A done'},certifiedContext:certifiedContext([claim('C-A','E-A',['A'])])});
...
test('Multi-obligation completion is deterministic when Root maps confirmed Claims to every obligation',async()=>{
  ...
  const result=await verifier.review({task:task(),proposal:{finalResult:'A、B 已完成',summary:'done'},certifiedContext:certifiedContext([claim('C-A','E-A',['A']),claim('C-B','E-B',['B'])])});
...
test('Even a single canonical goal requires Root to state the completion relation explicitly',async()=>{
  const text='完成当前目标';
  const single={id:'T-2',requirementSources:[{id:'REQ-T-2-0001',text}],taskContract:{obligations:[{id:'OBL-T-2-GOAL',certification:'supported',requirementRefs:[{sourceId:'REQ-T-2-0001',start:0,end:text.length}],criterion:{mode:'outcome',acceptedOutcomes:['succeeded']}}]}};
  const verifier=new CompletionAssessmentVerifier();
  const unmapped=await verifier.review({task:single,proposal:{finalResult:'done'},certifiedContext:certifiedContext([claim('C-1','E-1')])});
  assert.equal(unmapped.assessments[0].certification,'unresolved');
  const mapped=await verifier.review({task:single,proposal:{finalResult:'done'},certifiedContext:certifiedContext([claim('C-1','E-1',['OBL-T-2-GOAL'])])});
  assert.equal(mapped.assessments[0].certification,'supported');
});

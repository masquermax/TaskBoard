...
test('work authority candidates cover only demanded missing semantics and preserve immutable requirement refs',()=>{
  ...
  const candidates=authoritySemanticCandidatesForWork(task,[{projectAccess:'write',networkAccess:false}]);
...
test('work authority certification runs once; persisted authority prevents Validator replay after a new Runtime session',async()=>{
  ...
  const workUnits=[{projectAccess:'write',networkAccess:false}];
  const certified=await runtime.certifyWorkAuthority(base,session,{onTaskContractAuthority:value=>{persisted=value;},onProgress(){},onExecutionStarted(){}},workUnits);
  ...
  await runtime.certifyWorkAuthority(restored,restoredSession,{onTaskContractAuthority(){throw new Error('must not persist twice');},onProgress(){},onExecutionStarted(){}},workUnits);

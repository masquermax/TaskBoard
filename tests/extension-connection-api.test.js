...
test('connection validation, profile identity, busy and apply states map to stable HTTP errors',async()=>{
  ...
  ]){const handler=createExtensionConnectionHandler({connectionSettings:{getPublic(){return{};},async update(){throw new Error(message);}}});const res=response();await handler(request('PUT',{action:'selectProfile',profileId:'missing'},{'x-taskboard-action':'ui'}),res);assert.equal(res.out.status,status);assert.equal(JSON.parse(res.out.body).error,message);}
});

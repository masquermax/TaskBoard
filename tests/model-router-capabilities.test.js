...
test('broad or deep Work is not made lightweight merely by having a stopCondition',async()=>{
  const provider=new Provider({discoveryLevel:'full',routingSafe:true,defaults:{model:'frontier'},models:models()}),router=new ModelRouter({capabilityProvider:provider}),current=task({projectScopes:[{path:'/repo'}]});await router.prepare({role:'root',task:current});
...
test('complex Root is capped at the normal high band instead of auto-escalating to exotic tiers',async()=>{
  const provider=new Provider({discoveryLevel:'full',routingSafe:true,defaults:{model:'frontier'},models:models()}),router=new ModelRouter({capabilityProvider:provider}),current=task({title:'复杂架构综合分析',instruction:'完整检查架构、安全、性能、并发、迁移并给出端到端重构方案'});await router.prepare({role:'root',task:current});const route=router.route({role:'root',task:current});assert.equal(route.model,'frontier');assert.equal(route.reasoningEffort,'high');assert.ok(!['xhigh','max','ultra'].includes(route.reasoningEffort));
});

test('unsafe/non-selectable capability leaves model choice to Executor',async()=>{
  const provider=new Provider({discoveryLevel:'basic',routingSafe:false,modelSelection:{explicitPerTurn:false,maxPerTurn:1},defaults:{model:'configured'},models:models()}),router=new ModelRouter({capabilityProvider:provider}),current=task();await router.prepare({role:'root',task:current});const route=router.route({role:'root',task:current});assert.equal(route.model,null);assert.equal(route.reasoningEffort,null);assert.equal(route.routeReason,'executor-default');
});

test('Root capability discovery never receives Project cwd; scoped Subagent discovery may',async()=>{
  const provider=new Provider({discoveryLevel:'partial',routingSafe:false,defaults:{model:null},models:[]}),router=new ModelRouter({capabilityProvider:provider}),current=task({projectScopes:[{path:'/project/a'}]});
  await router.prepare({role:'root',task:current});assert.deepEqual(provider.calls[0],{});
  await router.prepare({role:'subagent',task:current});assert.deepEqual(provider.calls[1],{context:{cwd:'/project/a'}});
});

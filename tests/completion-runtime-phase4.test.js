...
test('mapped confirmed completion reaches goal_satisfied in the same Root turn',async()=>{const x=runtime(),outcome=await x.runtime.execute(task({mapped:true}));assert.equal(outcome.kind,'goal_satisfied');assert.equal(outcome.goalState,'satisfied');assert.equal(x.rootTurns(),1);});

test('unmapped completion fails closed after one Root turn instead of completion repair',async()=>{const x=runtime();await assert.rejects(x.runtime.execute(task()),/ROOT_INVALID_COMPLETION_DECISION/);assert.equal(x.rootTurns(),1,'the same state is never sent back to Root as completion feedback');});

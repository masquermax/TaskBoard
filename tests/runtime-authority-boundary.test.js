...
class CaptureClient{constructor(){this.calls=[];}async runTurn(request){this.calls.push(request);return JSON.stringify({kind:'complete',summary:'ok',stageResult:null,finalResult:'ok',resultMode:'execution',evidence:[],claims:[],gaps:[],recommendations:[],steps:[],gateway:null,gapResolutions:[],delegations:[]});}}
...
test('completed Work Unit receipt survives process/session boundaries without becoming public Task context',()=>{
  ...
  try{
    ...
    const receipt={id:'WU-1',signature:'sig-1',workUnit:{id:'WU-1',title:'scan',goal:'scan',expectedOutput:'result',stopCondition:'done',projectAccess:'none',networkAccess:false,skillId:null,dependsOn:[],inputRefs:[]},result:{delegationId:'WU-1',result:'done',evidence:[],blocker:null},completed_at:'2026-08-13T00:00:00.000Z'};
...
test('GovernanceCompiler emits executable AuthorizedGrant only for Root and Subagent',()=>{
  const compiler=new GovernanceCompiler(),task={id:'T',projectScopes:[{path:'/project/a'}],taskContract:{authority:{}}};
  const root=compiler.compileForRole(task,'root');
  const subagent=compiler.compileForRole(task,'subagent',{workUnit:{id:'WU',projectAccess:'read',networkAccess:false,inputRefs:['project:0']}});
  ...
  assert.equal(compiler.compileForRole(task,'validator').authorizedGrant.role,'validator');
  assert.equal(compiler.compileForRole(task,'validator').authorizedGrant.projectAccess,'none');
});
...
test('CodexExecutor projects Root grant to a runtime boundary with no Project/network access',async()=>{
  ...
  try{
    const task=taskWithProject(project),policyContext=new GovernanceCompiler().compileForRole(task,'root');
    await executor.runRoot({task,subagentResults:[],humanGatewayHistory:[],certifiedContext:{claims:[],gaps:[],unresolvedObligations:[]},modelPolicy:{},policyContext});
    const call=client.calls[0];assert.equal(call.permissionProfile,'taskboard_runtime');assert.equal(call.runtimeWorkspaceRoots.includes(project),false);assert.equal(call.networkAccess,false);
...
test('Root sees a Stage only after every issued sibling Work Unit has finished',async()=>{
  ...
  const executor={
    async runRoot({subagentResults}){
      rootTurns+=1;
      if(rootTurns===1)return{kind:'delegate',summary:'分工',stageResult:null,finalResult:null,resultMode:'execution',evidence:[],claims:[],gaps:[],recommendations:[],steps:[],gateway:null,gapResolutions:[],delegations:[
      ...
      return{kind:'complete',summary:'done',stageResult:null,finalResult:'done',resultMode:'execution',evidence:[],claims:[],gaps:[],recommendations:[],steps:[],gateway:null,gapResolutions:[],delegations:[]};
    },
  ...
  const router=new ModelRouter(),subagent=new SubagentRuntime({executor,modelRouter:router}),runtime=new RootRuntime({...successfulCompletionDependenciesForControlFlowTest(),executor,modelRouter:router,subagentRuntime:subagent,maxConcurrentSubagents:2});

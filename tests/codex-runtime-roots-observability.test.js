...
test('runtime roots mismatch records requested and returned roots before failing closed', async () => {
  ...
  const runtimeRoot=process.platform==='win32'?'D:\\workspace\\validator':'/workspace/validator';
  ...
  await assert.rejects(client.runTurn({
    ...
    diagnosticContext:{taskId:'T-ROOTS',workUnitId:null,role:'validator'},
  ...
  assert.equal(mismatch.role,'validator');

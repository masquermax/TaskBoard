...
test('model schemas contain only Root and Subagent owned controls',()=>{
  assert.ok(rootSchema.properties.delegations);assert.ok(rootSchema.properties.gateway);assert.equal('stageResult' in rootSchema.properties,false);
  assert.deepEqual(Object.keys(subagentSchema.properties).sort(),['blocker','delegationId','evidence','result']);
  for(const key of ['claims','gaps','recommendations','delegations','gateway','findings','discoveries','uncertainty'])assert.equal(key in subagentSchema.properties,false);
});

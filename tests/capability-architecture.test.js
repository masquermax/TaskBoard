...
import { loadGovernanceDocuments } from '../src/governance/governance-loader.js';
...
const expectedContracts=['SCHEDULER','ROOT','WORK_UNIT','SUBAGENT','VALIDATOR','TASK_CORE','HUMAN_GATEWAY','SKILL','TOOL_EXECUTOR','UI_SURFACE'];
...
test('Capability Map names one owner for each critical decision and explicitly leaves Project Knowledge unimplemented',()=>{
  ...
  for(const pair of [
    ['Task 生命周期状态','Scheduler'],
    ['Task 目标理解','Root Agent'],
    ['delegated Work Unit 的具体执行','Subagent'],
    ['Root Candidate Delta 是否可成为正式 Task 结论','Validator'],
    ['History 边界','Validator'],
    ['原子持久化','Task Core'],
    ['某类工作“怎么做”','Skill'],
  ]){
    assert.match(map,new RegExp(`${pair[0].replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}[^\\n]*${pair[1].replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}`));
  }
  assert.match(map,/Project Knowledge[\s\S]*仍没有独立、可认证、可复用/i);
});
...
test('system governance remains available to maintenance, while ordinary role context is projected from the owned Capability only',()=>{
  const docs=loadGovernanceDocuments(rootDir);
  assert.equal('analysisRules' in docs,false);
  assert.ok(docs.constitution.length>0);
  assert.ok(docs.adrs.length>0,'ADRs remain available as engineering decision memory');
  const runtimeCompiler=new GovernanceCompiler({rootDir});
  assert.equal('adrs' in runtimeCompiler.documents,false,'ADR is engineering memory, not a hidden Runtime authority surface');
  const oldRules=readFileSync(resolve(rootDir,'docs/ANALYSIS_RULES.md'),'utf8');
  assert.match(oldRules,/Status:\s*SUPERSEDED/i);
  assert.doesNotMatch(oldRules,/^##\s+AR-\d+\s+🔒/m);
});
...
test('owner-sensitive durable mutations stay behind their declared Capability owner surfaces',()=>{
  ...
  for(const method of ['commitProgressHistory','addProgressHistory','updateStageResult']){
...
test('Capability implementation map points only to existing current source files and no removed role entry',()=>{
  const map=readFileSync(resolve('docs/CAPABILITY_IMPLEMENTATION_MAP.md'),'utf8');

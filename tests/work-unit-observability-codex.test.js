...
test('Codex streams tool timing, then SubagentRuntime may finalize one compact Work Unit summary',async()=>{
  ...
  try{
    ...
    assert.ok(tool);assert.equal(tool.operationClass,'search');assert.equal(tool.success,true);assert.ok(tool.resultBytes>0);assert.equal(events.some(item=>item.event==='work-unit-summary'),false);
    finalizeWorkUnitObservability({taskId:'T-0009',workUnitId:'authority-evidence',evidence:[{id:'E-1'}],status:'completed'});
    events=parseDiagnostics(diagnostics);const summary=events.find(item=>item.event==='work-unit-summary');assert.ok(summary);assert.equal(summary.toolCallCount,1);assert.equal(summary.evidenceCount,1);assert.equal('postSaturationCalls' in summary,false);

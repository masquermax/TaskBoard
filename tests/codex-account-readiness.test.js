...
test('re-applying the already-selected Codex account revalidates auth and replaces stale capability readiness',async()=>{
  ...
  assert.deepEqual(requests[1],{method:'config/read',params:{includeLayers:true}},'account revalidation must inspect config layers so inherited provider overrides cannot masquerade as the builtin OpenAI provider');
